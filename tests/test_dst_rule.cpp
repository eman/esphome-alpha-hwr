...
// ── The three defects this feature was withdrawn from #287 for ──────────────

// 1. A transition at LOCAL MIDNIGHT used to name the wrong day.
//
// The date fields came from one second before the transition while the hour
// wrapped to 0, so the rule described the previous civil day. A sweep of the tz
// database put this at 212 start-side and 174 end-side wrong answers -- Havana,
// Cairo, Beirut, the Azores, Santiago, Nuuk. Both directions hurt: a correctly
// configured pump was reported broken (and the entity printed the node's wrong
// rule, so a user who followed it would set the pump a day early), and a pump
// misconfigured to the probed rule reported AGREE.
//
// POSIX TZ strings, so these are not tzdata artefacts -- the string says what
// the answer must be.
static void test_a_transition_at_local_midnight_names_the_right_day() {
  std::cout << "\n=== A midnight transition names the right day ===" << std::endl;

  // Cuba: second Sunday of March at 00:00, first Sunday of November at 01:00.
  with_tz("CST5CDT,M3.2.0/0,M11.1.0/1", []() {
    const DstRule h = probe_host_dst_rule(2026);
    TEST_ASSERT(h.valid && h.enabled, "the zone observes DST");
    TEST_ASSERT(h.start.weekday == 7 && h.start.occurrence == 2 && h.start.hour == 0,
                "the start is the SECOND SUNDAY at 00:00, as M3.2.0/0 says -- "
                "not the first Saturday, which is the previous civil day");
    TEST_ASSERT(resolve_transition_day(h.start, 2026) == 8,
                "March 2026's second Sunday is the 8th");
  });

  // An EU-rule zone at UTC-1, which puts the EU transition at local midnight.
  with_tz("WET0WEST,M3.5.0/0,M10.5.0/1", []() {
    const DstRule h = probe_host_dst_rule(2026);
    TEST_ASSERT(h.start.weekday == 7,
                "the Azores start on a SUNDAY, like every other EU zone");
    TEST_ASSERT(resolve_transition_day(h.start, 2026) == 29,
                "and on March 29, the last Sunday -- not the 28th");
  });

  // The control: a zone whose transition is NOT at midnight was always right,
  // and must stay right.
  with_tz("PST8PDT,M3.2.0/2,M11.1.0/2", []() {
    const DstRule h = probe_host_dst_rule(2026);
    TEST_ASSERT(h.start.weekday == 7 && h.start.occurrence == 2 && h.start.hour == 2,
                "US Pacific is unchanged: second Sunday of March at 02:00");
  });
}

// 2. A year with ONE transition is not a rule this can express.
//
// It used to emit `enabled` with one half left {0,0,0,0}, printed to the user
// as "? ?#0 00:00" -- fields the decoder would have rejected outright on the
...
//
// The probe now keys on the offset direction rather than on an is_dst flag,
// which is also what makes the southern hemisphere come out in the right order.
// Keying on the flag reported "+0" with start and end reversed
// (America/Grand_Turk 2018, when the zone was redefined).
static void test_an_offset_that_never_moves_is_not_a_transition() {
  std::cout << "\n=== A zone whose offset never moves reports no DST ===" << std::endl;
  with_tz("MST7", []() {
    const DstRule h = probe_host_dst_rule(2026);
    TEST_ASSERT(h.valid && !h.enabled, "a fixed-offset zone observes no DST");
    TEST_ASSERT(h.offset_minutes == 0, "and reports no shift");
  });
}

// 4. The zone comes from ESPHome, not from libc (issue #289).
//
// This is the half that made the feature inert on the device: the probe was all
// libc, and ESPHome sets libc's zone only under USE_HOST -- so on an ESP32 it
// found no transitions and reported a mismatch for EVERY user in a DST zone.
//
// Under MockZoneOverride the two disagree: libc is told a DST zone, ESPHome a
// fixed offset. A probe reading libc would find the US rule; one reading
// ESPHome finds no DST.
static void test_the_probe_reads_esphomes_zone_not_libcs() {
  std::cout << "\n=== The probe reads ESPHome's zone, not libc's ===" << std::endl;
  setenv("TZ", "PST8PDT,M3.2.0/2,M11.1.0/2", 1);  // libc: US Pacific, WITH DST
  tzset();
  {
    esphome::MockZoneOverride fixed(-8 * 3600);  // ESPHome: fixed, no DST
    const DstRule h = probe_host_dst_rule(2026);
    TEST_ASSERT(h.valid, "the probe answers");
    TEST_ASSERT(!h.enabled,
                "and finds NO DST, because ESPHome's zone has none -- a probe "
                "reading libc would have found the US rule here, which is what "
                "shipped and what made this inert on the device");
  }
  // The control: with the override gone it follows libc again, so the assertion
  // above is about the zone source and not about the arithmetic.
  const DstRule h = probe_host_dst_rule(2026);
  TEST_ASSERT(h.valid && h.enabled,
              "back on libc's zone the same call finds the US rule");
  setenv("TZ", "UTC", 1);
  tzset();
}
...
int main() {
  ...
  test_a_transition_at_local_midnight_names_the_right_day();
  test_a_year_with_one_transition_is_not_expressible();
  test_an_offset_that_never_moves_is_not_a_transition();
  test_the_probe_reads_esphomes_zone_not_libcs();
