...
- **Constant Flow abort bug** —
  Fixed an issue in `with_resolved_setpoint` where `CONSTANT_FLOW` changes would hard-abort instead of proceeding with the setpoint write (PR #89).
- **Transport Sub-ID Matching bug** —
  Fixed `send_apdu_command` calls in the control service that were inadvertently passing the Object ID as the Sub ID due to reversed argument order (PR #89).
## [0.10.1] - 2026-07-11
