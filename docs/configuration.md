...
| `pump_on_continuation_max_seconds` | int | `600` | s — how long the continuation tier may keep asserting a draw that nothing has measured since. It only ever runs out while the subtraction is unavailable, because a subtraction at or below `pump_on_demand_flow_threshold` releases the tier on the spot and retires the capture. The case it bounds is a pump turning below `pump_on_demand_min_speed_rpm`, where no measurement of household draw exists at all and the claim would otherwise stand for the whole run. `0` disables the tier rather than unbounding it |
