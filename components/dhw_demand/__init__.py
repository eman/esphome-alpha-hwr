...
DhwDemandComponent = dhw_demand_ns.class_("DhwDemandComponent", cg.PollingComponent)
...
_RETIRED_KEYS = {
    "inlet_pressure_transient_threshold": ("the pressure-transient vote fired on the pump's own speed changes"),
    ...
    "pump_power_spike_threshold": ("the power-spike vote had the same defect as the current vote it corroborated"),
    "pump_head_rate": (
    ...
    "inlet_pressure": ("inlet pressure was read only by the two pressure votes"),
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_PUMP_OFF_CURRENT_THRESHOLD, default=0.03): cv.positive_float,
        cv.Optional(CONF_FLOW_THRESHOLD, default=0.3): cv.positive_float,
        cv.Optional(CONF_THERMAL_COLLAPSE_RATE, default=0.05): cv.positive_float,
        cv.Optional(CONF_DHW_CHARGE_DROP_RATE, default=0.005): cv.positive_float,
        cv.Optional(CONF_PUMP_ON_DEMAND_FLOW_THRESHOLD, default=0.3): cv.positive_float,
        ...
        cv.Optional(CONF_PUMP_ON_DEMAND_MIN_SPEED_RPM, default=1950.0): cv.positive_float,
        ...
        cv.Optional(CONF_PUMP_ON_DEMAND_MAX_STALE_SECONDS, default=30): cv.int_range(min=1, max=3600),
        cv.Optional(CONF_FLOW_MAX_STALE_SECONDS, default=60): cv.int_range(min=1, max=3600),
        # Bounded for the same uint32_t wrap reason as the staleness keys,
        # and 0 is legal and meaningful: "high right now is enough".
        cv.Optional(CONF_DHW_IN_USE_MIN_SECONDS, default=70): cv.int_range(min=0, max=3600),
        cv.Optional(CONF_FLOW_LATCH_SECONDS, default=30): cv.positive_int,
        ...
        cv.Optional(CONF_LATCH_PUMP_OFF_SUPPRESSION_SECONDS, default=30): cv.int_range(min=0, max=3600),
        ...
        cv.Optional(CONF_PUMP_ON_SETTLE_SECONDS, default=10): cv.int_range(min=0, max=3600),
        ...
        cv.Optional(CONF_PUMP_ON_CONTINUATION_MAX_SECONDS, default=300): cv.int_range(min=0, max=3600),
        cv.Optional(CONF_SESSION_GAP_TOLERANCE_SECONDS, default=60): cv.positive_int,
        cv.Optional(CONF_DEMAND_RELEASE_SECONDS, default=30): cv.positive_int,
        **{cv.Optional(key): _retired(key) for key in _RETIRED_KEYS},
        **{cv.Optional(old): _renamed(old) for old in _RENAMED_KEYS},
    }
...
async def to_code(config):
    ...
    # ── Thresholds ─────────────────────────────────────────────────────────────
    cg.add(var.set_pump_off_current_threshold(config[CONF_PUMP_OFF_CURRENT_THRESHOLD]))
    cg.add(var.set_flow_threshold(config[CONF_FLOW_THRESHOLD]))
    cg.add(var.set_thermal_collapse_rate(config[CONF_THERMAL_COLLAPSE_RATE]))
    cg.add(var.set_dhw_charge_drop_rate(config[CONF_DHW_CHARGE_DROP_RATE]))
    cg.add(var.set_pump_on_demand_flow_threshold(config[CONF_PUMP_ON_DEMAND_FLOW_THRESHOLD]))
    cg.add(var.set_pump_on_demand_min_speed_rpm(config[CONF_PUMP_ON_DEMAND_MIN_SPEED_RPM]))
    cg.add(var.set_pump_on_demand_max_stale_seconds(config[CONF_PUMP_ON_DEMAND_MAX_STALE_SECONDS]))
    cg.add(var.set_flow_max_stale_seconds(config[CONF_FLOW_MAX_STALE_SECONDS]))
    cg.add(var.set_dhw_in_use_min_seconds(config[CONF_DHW_IN_USE_MIN_SECONDS]))
    cg.add(var.set_flow_latch_seconds(config[CONF_FLOW_LATCH_SECONDS]))
    cg.add(var.set_latch_pump_off_suppression_seconds(config[CONF_LATCH_PUMP_OFF_SUPPRESSION_SECONDS]))
    cg.add(var.set_pump_on_demand_settle_seconds(config[CONF_PUMP_ON_SETTLE_SECONDS]))
    cg.add(var.set_pump_on_continuation_max_seconds(config[CONF_PUMP_ON_CONTINUATION_MAX_SECONDS]))
    cg.add(var.set_session_gap_tolerance_seconds(config[CONF_SESSION_GAP_TOLERANCE_SECONDS]))
    cg.add(var.set_demand_release_seconds(config[CONF_DEMAND_RELEASE_SECONDS]))
