...
> **The 0.3 GPM floor discards real draws, and that is not currently justified by measurement.** Observed while benching the continuation tier: a steady household draw metering 0.11–0.16 GPM produced `deterministic_idle` throughout — no detection at all, pump off, meter working. Over 14 h of one installation's history, 24 sub-threshold samples fell in `0 < flow < 0.30` **with the pump confirmed off**, i.e. with the meter seeing only genuine demand.
>
> Sensor resolution is not the limit: the meter reports a hard `0.0` at rest rather than dithering, so the band is real signal rather than noise. The actual risk in lowering it is recirculation decay — loop flow runs a median 1.45 GPM pre-shutdown and collapses *through* any threshold on its way down, so a lower floor widens the window in which the pump's own dying flow reads as a draw. `latch_pump_off_suppression_seconds` disarms the *latch* in that window but does not suppress a direct flow vote.
>
> So this is a measurement question, not a tuning preference: what is needed is the distribution of pump-off meter readings in the 0.05–0.30 band, split by proximity to a pump-off edge. Until that exists the floor stays at 0.3. Tracked in issue #180; do not lower it on intuition.
