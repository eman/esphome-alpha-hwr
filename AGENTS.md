...
3. **Confirm readbacks decide the verdict** — a write settles as `accepted`/`clamped`/`rejected` based on what the pump reports actually holding, not on the ACK (the two-phase-commit window of §8.1 often closes without a matchable ACK even on success). The two config writes were the exception until issue #234: a missing ACK settled `rejected` before any readback ran, which asserts a refusal nothing at that point has evidence for. A short ACK carries no sequence number and no object echo, so it cannot be attributed to a particular write in either direction — an unanswered write is now settled by the readback like every other, with the silence reported in `detail`.
